...
import com.zhihaofans.androidbox.entity.MySection
import com.zhihaofans.androidbox.entity.Status
import com.zhihaofans.androidbox.entity.Video
...
open class DataServer private constructor() {
    ...
    companion object {
        ...
        fun getMultipleItemData(): List<MultipleItem> {
            return mutableListOf<MultipleItem>().apply {
                for (i in 0..4) {
                    add(MultipleItem(MultipleItem.IMG, MultipleItem.IMG_SPAN_SIZE))
                    add(MultipleItem(MultipleItem.TEXT, MultipleItem.TEXT_SPAN_SIZE, CYM_CHAD))
                    add(MultipleItem(MultipleItem.IMG_TEXT, MultipleItem.IMG_TEXT_SPAN_SIZE))
                    add(MultipleItem(MultipleItem.IMG_TEXT, MultipleItem.IMG_TEXT_SPAN_SIZE_MIN))
                    add(MultipleItem(MultipleItem.IMG_TEXT, MultipleItem.IMG_TEXT_SPAN_SIZE_MIN))
                }
            }
        }
